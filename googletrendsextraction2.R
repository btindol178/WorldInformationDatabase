...
# 
#   }
#   tempouter$hits <- gsub("<1",0,tempouter$hits);tempouter$hits <- as.integer(tempouter$hits)
#   tempouter = unique(tempouter) # Remove duplicates after all the mess
#   #data_wide <- dcast(tempouter, date + time +geo ~ keyword, value.var="hits")
#   #data_wide <- merge(mapping,data_wide,by=c("geo"),all.x=TRUE)
#   #colnames(data_wide)[1] <-"geo";colnames(data_wide)[2] <-"state";colnames(data_wide)[3] <-"state_code";colnames(data_wide)[4] <-"country";colnames(data_wide)[5] <-"weekof";colnames(data_wide)[6] <-"date";
#   #data_wide$id = paste0(data_wide$state,"_",data_wide$date)
...
# make final dataframe in to variables
trends1 <- temp_3
trends2 <- temp_4
trends3 <- temp_5

# THIS FUNCTION WILL TAKE DATAFRAME AND 
trends3 = read.csv("C:/Users/btindol/OneDrive - Stryker/Python Scripts/GlobalDatabaseExtractionToMYSQL/CustomerGlobalIntelligenceScripts/trends3.csv",encoding = "ISO-8859-1")
trends2 = read.csv("C:/Users/btindol/OneDrive - Stryker/Python Scripts/GlobalDatabaseExtractionToMYSQL/CustomerGlobalIntelligenceScripts/trends2.csv",encoding = "ISO-8859-1")
trends1 = read.csv("C:/Users/btindol/OneDrive - Stryker/Python Scripts/GlobalDatabaseExtractionToMYSQL/CustomerGlobalIntelligenceScripts/trends1.csv",encoding = "ISO-8859-1")


trends3 <- trends3[-c(1)]
trends3$date <- as.Date(trends3$date)
...
makedailytrends <- function(df){
  ...
  trends5 <- trends4 %>%
    group_by(geo,keyword)%>%
    mutate(hits = na.approx(hits,na.rm=FALSE,maxgap = 10, rule = 2))
  
  mapping = read.csv("C:/Users/btindol/OneDrive - Stryker/Python Scripts/GlobalDatabaseExtractionToMYSQL/CustomerGlobalIntelligenceScripts/stateabbriviationmapping.csv",encoding = "ISO-8859-1");colnames(mapping) <-c("state","state code","country","geo");
  trends6 <- merge(trends5,mapping,by=c("geo"),all.x=TRUE)
  trends6 = trends6[,c(9,2,3,5,11)];colnames(trends6)[3] <- "date"
  return(trends6)
}

finaldfz1 <- makedailytrends(trends1)
finaldfz2 <- makedailytrends(trends2)
finaldfz3 <- makedailytrends(trends3)
...
#         time = daterange$daterange[j]
#         print(paste("for time range",time))
#         res1 <- gtrends(c(var), geo=state,time = time)#$interest_by_city IF YOU WANT INFORMATION BY CITY 
#         res1 <- data.frame(res1$interest_over_time)  # make that iteration of res1 a dataframe 
#         tempinner <- rbind(tempinner,res1) # bind the dataframe to tempinner
#         print(paste("done inner loop for var",var,"for state",state,"in time range",time))
#       }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
